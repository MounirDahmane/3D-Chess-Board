...
public:
    ...
    // render the mesh
    void Draw(MyShader &shader, unsigned int stencil) const
    {
        ...
        // draw mesh
        if (stencil == NONE)// without stencil effect
        {
            glBindVertexArray(VAO);
            glDrawElements(GL_TRIANGLES, static_cast<unsigned int>(indices.size()), GL_UNSIGNED_INT, 0);
            glBindVertexArray(0);
        }
        else if(stencil == FIRST)
        {
        ...
        else if (stencil == SECOND)
        {
            // do not forget to reset the model matrix->Hint : do the Scaling greater then 1.0f
